...
void IRAM_ATTR onTimer() {
  ...
  if ((conta_interrupt == 0) & (conta_fim !=0) )digitalWrite(pino_pwm,true);
  if (conta_interrupt == conta_fim) digitalWrite(pino_pwm,false);
  conta_interrupt = conta_interrupt + 1;
  if (conta_interrupt >99) conta_interrupt = 0;
...
void saida_pwm(float valor_em_percentagem)
{
  ...
  if (valor_em_percentagem > 99.9) factor44 = 100.0;
  aux44 = int (factor44)+1; 
  if (valor_em_percentagem < 9.9) aux44 = 0;
...
// attach the channel to the GPIO to be controlled
  ...
  display.drawString(4, 44, String("EVSE 0.1.1"));
  display.display();
...
if (EEPROM.read(1)==0) errodiodo_paracarga = false;
else errodiodo_paracarga = true;




if (amperes > 32) amperes = 6;
...
void loop() {
  ...
  if (millis() - timer4 > 100)
  {
    ...
    */
    ...
    //Estado B
    if ((leitura_piloto_max3 > 3050) & (leitura_piloto_max3 < 3600))
    {
      t[1].in = true;
    ...
    //Estado C e D
    if ((leitura_piloto_max3 > 1400) & (leitura_piloto_max3 < 3050) /*& (t[10].q)*/)
    {
    ...
    //Estado erro Diodo
    if (((estado == 1) | (estado == 2)) & (teste_piloto == 0))
    {
  ...
  */
      ...
      //Estado erro Geral
      if ((leitura_piloto_max3 < 1400) & (teste_piloto == 0) /*& (!(!t[10].q) & (estado==0))*/)
      {
        t[12].in = true;
  ...
  if ((estado == 2) | ((estado == 4) & !errodiodo_paracarga  )   ) {
    ...
    if (t[4].q) { 
      ...
      //t[10].in=false;
      if (estado == 2) sprintf(linha1, "A carregar");
      if (estado == 4) sprintf(linha1, "Erro diodo");
      }
  ...
  if ((estado == 0) ) auxpwm = 100.0;

  if (t[2].q) auxpwm = float(amperes) / 0.6;
  
  if (inibir_carga) auxpwm = 0.0;
  ...
  if (teste_piloto ==1 ) auxpwm = 10.0 / 0.6;  //PWM 10A
  if (teste_piloto ==2 ) auxpwm = 100.0;       //+12V
  if (teste_piloto ==3 ) auxpwm = 0.0;         //-12V
  ...
  if (millis() - timer2 > 20)
  {
    ...
    if (digitalRead(pino_rele)) display.drawString(60, 22, String("|"));
    else display.drawString(60, 22, String("O"));
    ...
    //                      x   y    w   h  %
    display.drawProgressBar(0, 24, 46, 8, janelamin);
    display.drawProgressBar(0, 32, 46, 8, janelamax);
    ...
    if (t[6].q)
    {
      ...
      if (piscar) display.drawString(48, 22, String("#"));
      toque = 2;
    }
    else
    {
    ...
    if ((menu == 1) & ((toque == 0) || (toque ==1))  & (!bloquear) & (!escolher_amperes)) {
      ...
      display.drawString((24+(-1*(contadisplay % 144))), 44, String("Memorizar Corrente"));
    ...
    //timeout
    if (toque==0) t[9].in=true; else t[9].in=false;
  ...
  if (millis() - timer1 > 1000)
  {
    ...
    if (t[4].q) tempocarga=tempocarga+1;
