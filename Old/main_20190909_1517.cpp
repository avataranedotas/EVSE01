...
void loop()
{
  ...
  if (millis() - timer4 > 100)
  {
    ...
    //Estado A
    if ((leitura_piloto_max3 > MIN_PWM_A) && (!ciclo_estadoB) )
    {
      estado = 0;
    ...
    */
    ...
    //Entrar no Estado B
    if ((leitura_piloto_max3 > MIN_PWM_B) & (leitura_piloto_max3 < MIN_PWM_A) && (estado!=1) && (estado!=11) && (estado!=12))
    {
      t[1].in = true;

      if (t[1].q == true) //entra no estado 1, vai fazer ciclo de 2 segundos a -12V
      {
        estado = 1;
    ...
    //sair do ciclo de 2 segundos a -12V
    if ( (t[18].q) && (estado==1) ){
      ciclo_estadoB = false;
      estado = 11;
      t[18].in = false;
    ...
    //Continuar no Estado B (11)

    if (estado == 11) //saida pwm está a +12V
    {
  ...
  */
      ...
      //Estado erro Geral
      if ((leitura_piloto_max3 < MIN_PWM_C) & (!teste_pwm_inibir_errodiodo) & (!ciclo_estadoB) /*& (!(!t[10].q) & (estado==0))*/)
      {
        t[12].in = true;
    ...
    //Saida pwm regulacao EVSE, por defeito é +12V
    auxpwm = 100.0;

    if ((estado == 0))
      auxpwm = 100.0;

    //se o estado for 1 em ciclo low
    if ((estado==1) && (ciclo_estadoB)) auxpwm = 0.0;
    //if ((estado==1) && (ciclo_estadoB)) auxpwm = 0.0;

    //se o estado for 11
    if ((estado==11)) auxpwm = 100.0;

    //estado 12
    if ((estado==12)) auxpwm = float(amperes) / 0.6;



    //se o estado passar a 2 começa pwm
    if ((estado==2)) auxpwm = float(amperes) / 0.6;
