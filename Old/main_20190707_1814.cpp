...
String SendHTML(uint8_t led1stat,uint8_t led2stat){
  ...
  ptr +="</head>\n";
  ptr +="<body>\n";
  ptr +="<h1>EVSE Web Server</h1>\n";
  ptr +="<h3>Version "+String(VERSAO)+"</h3>\n";
...
void handle_mais_corrente() {
  ...
  servidor.send(200, "text/html", SendHTML(true,3)); 
}

void handle_menos_corrente() {
  Serial.println("Handel Menos corrente");
  servidor.send(200, "text/html", SendHTML(false,2)); 
}

/*
void handle_led2on() {
  Serial.println("GPIO5 Status: ON");
  servidor.send(200, "text/html", SendHTML(0,true)); 
}

void handle_led2off() {
  //LED2status = LOW;
  Serial.println("GPIO5 Status: OFF");
  servidor.send(200, "text/html", SendHTML(0,false)); 
}
*/
